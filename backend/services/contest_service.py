...
async def generate_contest(relationship_id: str, contest_type: str = "weekly") -> dict:
    ...
    rel_data = rel.data
    user_a = db.table("profiles").select("display_name").eq("id", rel_data["user_a_id"]).single().execute()
    user_b = db.table("profiles").select("display_name").eq("id", rel_data["user_b_id"]).single().execute()
    
    name_a = user_a.data["display_name"] if user_a.data else "Partner A"
    name_b = user_b.data["display_name"] if user_b.data else "Partner B"
...
async def submit_answer(question_id: str, user_id: str, answer: str) -> dict:
    ...
    question = db.table("contest_questions").select("*").eq("id", question_id).single().execute()
    if not question.data:
        return {"error": "Question not found"}
    
    q_data = question.data
    contest = db.table("contests").select("*").eq("id", q_data["contest_id"]).single().execute()
    if not contest.data:
        return {"error": "Contest not found"}
    
    rel = db.table("relationships").select("*").eq("id", contest.data["relationship_id"]).single().execute()
    if not rel.data:
        return {"error": "Relationship not found"}
...
async def complete_contest(contest_id: str) -> dict:
    ...
    rel_id = contest.data["relationship_id"]
    rel = db.table("relationships").select("bond_points, care_score, contests_completed, contests_won") \
        .eq("id", rel_id).single().execute()
    
    if rel.data:
        new_bond = rel.data["bond_points"] + bond_points
        new_care = min(100, rel.data["care_score"] + (bond_points // 5))
        
        db.table("relationships").update({
            "bond_points": new_bond,
            "care_score": new_care,
            "contests_completed": rel.data["contests_completed"] + 1,
            "contests_won": rel.data["contests_won"] + (1 if total >= contest.data["max_points"] * 0.7 else 0)
    ...
    return {
        ...
        "is_synchronized": is_synchronized,
        "bond_points_awarded": bond_points,
        "passed": total >= contest.data["max_points"] * 0.5
    }
